# Plan: Write the ISBT 128 parser implementation prompt

## Context

`task.txt` asks me to **"create a prompt for following task"** — the task being a future project to
implement an ICCBBA/ISBT 128 barcode parser in C# and TypeScript, exposing `check()`/`parse()`, after
analyzing the standard documents in `documents/` and asking the user about anything unclear.

The user confirmed (via clarifying questions) that:
1. **This session's deliverable is the prompt document only** — no implementation code yet. The prompt
   will be used later to drive an implementation session (this one or a fresh one).
2. The future parser should cover the **full ISBT 128 standard** (all non-retired data structures from
   ST-001), not just a core subset.
3. `parse()` should **decode what's locally resolvable** from reference tables embedded in the provided
   docs, and leave fields as raw codes wherever decoding requires ICCBBA's external licensed databases
   (Product Description Code descriptions, Manufacturer codes, Facility ID lookups).

I've read all four source documents in `documents/`:
- `iccbba.md` — ISBT 128 Standard Technical Specification (ST-001, v6.2.2, 7575 lines). Core spec: data
  identifiers, all ~40 data structures, embedded reference tables (RT001–RT046ish), DIN check-character
  algorithm (Appendix A), bar code concatenation rules (§10), label examples (Appendix C).
- `ST-012-...md` — SEC (Single European Code) supplement, defines the SEC text/DS038 format.
- `ST-017-...md` — MPHO Medical Device / UDI supplement, defines DS034 (PPIC/DI), UDI parsing (§5),
  Basic UDI-DI mod-37,2 checksum (Appendix 1).
- `RT019.pdf` — Reference Table RT019 (Transfusion Transmitted Infection Marker, DS027), already
  reproduced in `iccbba.md` §3.1 Table 18 but with 2 extra reserved positions (13–18) visible only in
  the PDF.

The only remaining action is to **write the prompt document** to disk. Per the task's own instruction
("all files you download or create should be inside project folder in `.claude` folder"), it belongs at
`.claude/prompts/isbt128-parser-implementation-prompt.md`. No other files are created or modified in
this pass — README/.gitignore/actual code are requirements *described inside* the prompt for the future
implementation phase, not actions to take now.

## Action

Create `.claude/prompts/isbt128-parser-implementation-prompt.md` with the following content:
...
## 13. Open questions to raise with the user during implementation

These were not settled during prompt-writing and should be confirmed before/while implementing:
1. Should the eye-readable **`SEC: ...` text form** be accepted directly by `check()`/`parse()`
   alongside the DS038 electronic form, or is only the electronic/data-structure form in scope?
2. Exact **naming conventions** (namespace/package names, e.g. `Iccbba.Isbt128Parser` /
   `@iccbba/isbt128-parser`) and **NuGet/npm publishing** intentions, if any.
3. Whether **non-ICCBBA / nationally-defined data structures** (§2.5, `&` + a–z) that can appear
   trailing in a compound message should be surfaced as raw opaque segments or simply dropped/ignored
   when their length can't be determined.
4. Any specific **target framework/language versions** (.NET version, TS/Node version) to target.
````
...
After writing the file, re-read it back to confirm it rendered correctly (no broken code fences), and
report the file path to the user. No code is built or run in this pass — there's nothing executable to
verify yet, since the deliverable is documentation.
