...
{
    [GeneratedRegex(@"^SEC:\s*([A-Za-z0-9]{21})\s+([A-Za-z0-9]{19})\s*$")]
    private static partial Regex SecTextPattern();
    ...
    public static Dictionary<string, object?> DecodeSecContent(string content)
    {
        ...
        var countryIdentifier = content[..2];
        var tissueEstablishmentCode = content[2..8];
        var uniqueDonationNumber = content[8..21];
        var codingSystemIdentifier = content[21..22];
        var productCode = content[22..29];
        var splitNumber = content[29..32];
        var rawExpiry = content[32..40];
        var expiryDate = rawExpiry == "00000000" ? "" : $"{rawExpiry[..4]}-{rawExpiry[4..6]}-{rawExpiry[6..8]}";
